...
class ProductController extends Controller
{
    ...
    public function index2()
    {
        ...
        $maxPrice = Product::select('price')->max('price');
        $minPrice = Product::select('price')->min('price');
        return view('clothing.index', compact(['brands', 'genders', 'categories', 'maxPrice', 'minPrice', 'products']));
    }
    public function index3()
    {
        ...
        $maxPrice = Product::select('price')->max('price');
        $minPrice = Product::select('price')->min('price');
        return view('accesories.index', compact(['brands', 'genders', 'categories', 'maxPrice', 'minPrice', 'products']));
    }
    public function filter(Request $request)
    {
        if ($request->ajax()) {
            $products = Product::where('category', 'Shoes');
    ...
    {
        if ($request->ajax()) {
            $products = Product::where('category', 'Accesories');
            $query = json_decode($request->get('query'));
            $price = json_decode($request->get('price'));
            $gender = json_decode($request->get('gender'));
            $brand = json_decode($request->get('brand'));
    ...
    public function show2(Product $product)
    {
        ...
        return view('clothing.show', compact('product', 'sizes'));
    }
    public function show3(Product $product)
    {
        $sizes = Stock::where('product_id', '=', $product->id)
            ->get([
                'name',
        ...
        return view('accesories.show', compact('product', 'sizes'));
    }
    ...
    public function create(Request $request)
    {
        $this->validate(request(), [
            ...
            'category' => 'required|in:Shoes,Clothing,Accesories',
        ]);
